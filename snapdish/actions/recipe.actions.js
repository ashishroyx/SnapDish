...
`;
    ...
    const strapiRecipeData = {
      data: {
        title: normalizedTitle,
    description: [], // IMPORTANT: description is type 'blocks', it MUST be an array []
    cuisine: (cuisine || "other").toLowerCase(), // Enum must match your list exactly
    category: (category || "dinner").toLowerCase(), // Enum must match your list exactly
    ingredients: recipeData.ingredients || [],
    instructions: recipeData.instructions || [],
    imageUrl: imageUrl || "",
    isPublic: true,
    author: user.id,
    prepTime: Math.round(Number(recipeData.prepTime)) || 0, // Must be Integer
    cookTime: Math.round(Number(recipeData.cookTime)) || 0, // Must be Integer
    servings: Math.round(Number(recipeData.servings)) || 1, // Must be Integer
    nutrition: recipeData.nutrition || {},
    tips: recipeData.tips || [],
    substitutions: recipeData.substitutions || [],
    // Adding this line tells Strapi to bypass the Draft stage
...
// --- 1. SAVE RECIPE TO COLLECTION ---
    const saveResponse = await fetch(`${STRAPI_URL}/api/saved-recipes`, {
      ...
      body: JSON.stringify({
        data: {
          user: user.id,
          Recipe: recipeId, // Matches your schema.json attribute name
          savedAt: new Date().toISOString(),
...
// --- 2. REMOVE RECIPE FROM COLLECTION ---
export async function removeRecipeFromCollection(formData) {
  try {
    const user = await checkUser();
    ...
    // Search for the relation using Capital 'Recipe'
    const searchResponse = await fetch(
      `${STRAPI_URL}/api/saved-recipes?filters[user][id][$eq]=${user.id}&filters[Recipe][id][$eq]=${recipeId}`,
      {
        headers: {
    ...
    if (!searchResponse.ok) throw new Error("Failed to search saved recipes");
    ...
    const savedRecordId = searchData.data[0].id;

    // Execute the DELETE
    const deleteResponse = await fetch(`${STRAPI_URL}/api/saved-recipes/${savedRecordId}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${STRAPI_API_TOKEN}`,
      },
    ...
    if (!deleteResponse.ok) throw new Error("Failed to delete recipe from collection");
...
// }
  ...
  try {
    ...
    const query = new URLSearchParams({
      ...
      "populate[Recipe][populate]": "*", // MUST be Capital 'R'
      "sort": "createdAt:desc",
    });

    const response = await fetch(
      `${STRAPI_URL}/api/saved-recipes?${query.toString()}`,
      {
        headers: {
          Authorization: `Bearer ${STRAPI_API_TOKEN}`,
        },
    ...
    if (!response.ok) {
      ...
      console.error("❌ Strapi Fetch Error:", response.status, errorBody);
      throw new Error(`Failed to fetch saved recipes: ${response.status}`);
    }

    const data = await response.json();

    // Mapping the data using the Capital 'R'
    // const recipes = data.data
    //   .map((item) => {
    //     // Accessing item.attributes.Recipe (Capital R)
    //     const recipeWrapper = item.attributes?.Recipe?.data;
        
    //     if (!recipeWrapper) return null;

    //     return {
    //       id: recipeWrapper.id,
    //       ...(recipeWrapper.attributes || recipeWrapper),
    //       savedRecipeId: item.id, // ID of the bookmark itself
    //     };
    //   })
    //   .filter(Boolean);
    const recipes = data.data
  .map((item) => {
    // 1. Try v4 style (item.attributes.Recipe.data.attributes)
    // 2. Try v5 style (item.Recipe)
    const recipeData = item.attributes?.Recipe?.data || item.Recipe?.data || item.Recipe;
    
    if (!recipeData) return null;

    const attributes = recipeData.attributes || recipeData;
